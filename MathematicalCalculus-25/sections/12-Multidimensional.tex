...
\begin{thrm}{}{}
    \[
        f \in D(\ol{a}) \Rightarrow \lim\limits_{t \to 0} \frac{f(\ol{a} + t \ol{v}) - f(\ol{a})}{t} = df(\ol{a}) \cdot \ol{v}
    \]
\end{thrm}
...
\begin{defn}{Градиент}{}
    \[
        \nabla f (\ol{a}) = \left( \frac{\partial f}{\partial x_1}, \ldots, \frac{\partial f}{\partial x_d} \right)
    \]
\end{defn}
...
А равенство по КБШ достигается как раз при коллинеарности.

\begin{defn}{}{}
    \( \{ c \: \vline \: f(x, y) = c \} \) --- множество \( c \)-уровня.
\end{defn}
...
\begin{defn}{Матрица Якоби}{}
    ...
    Если \( f \in D(\ol{a}) \), то \( J_f (\ol{a}) = df(\ol{a}) \)
\end{defn}

...
\begin{thrm}{Предел сложной функции}{}
    Пусть \( f : (X, \rho_x) \to (Y, \rho_y) \), \( g : (Y, \rho_y) \to (Z, \rho_z) \)

    Пусть \( a \) --- предельная точка \( X \), а \( b \) --- предельная точка \( Y \)

    Пусть \( \lim\limits_{x \to a} f(x) = b \), \( \lim\limits_{x \to b} g(x) = c \)
    и \( f(x) \neq b \) в некоторой окрестности \( a \)

    Тогда \( \lim\limits_{x \to a} g(f(x)) = c \)
\end{thrm}

\( \forall \{ a_n \} \subset X : a_n \to a \lim\limits_{n \to \infty} f(a_n) = b \),
причем начиная с некоторого \( n \) \( f(a_n) \neq b \).

Поэтому можно считать, что это выполнено для всех \( n \).

Из условия знаем, что \( \lim\limits_{n \to \infty} g(f(a_n)) = c \).

Поскольку это верно \( \forall \{ a_n \} \),
доказали теорему.
...
Это верно \( \forall \{ a_n \} \), что и требовалось доказать.
...
\begin{example}{}{}
    \( f(x_1, x_2, x_3) = e^{x_1 x_2} \sin x_3 \). Найдем все частные производные:
    \begin{itemize}
        \item
...
По теореме Ролля \( \exists \xi \in (0, 1) : h'(\xi) = 0 \), то есть:
\begin{gather*}
...
\begin{defn}{}{}
    ...
    Пусть \( \forall \ol{x} \in \mathring{U} (\ol{a}) \) верно \( f(\ol{x}) \geq f(\ol{a}) \) (\( > f(\ol{a}) \))

    Точка \( \ol{a} \) называется точкой локального минимума функции \( f \) (строгого локального минимума).
    Если \( f(\ol{x}) \leq f(\ol{a}) \) (\( f(\ol{x}) < f(\ol{a})\)), то \( \ol{a} \) --- точка локального максимума (строгого).
\end{defn}

\begin{thrm}{Необходимое условие экстремума}{}
    Пусть \( a \) --- точка локального экстремума функции \( f \).
    Пусть в точке \( a \) определены все частные производные \( f \).

    Тогда \( \nabla f (\ol{a}) = \ol{0} \).
\end{thrm}

Рассмотрим функцию \( g(x_j) = f(a_1, \ldots, a_{j - 1}, x_j, a_{j + 1}, \ldots, a_d) \).
Тогда у \( g \) в точке \( a_j \) локальный экстремум и \( g \in D(a_j) \)
\( \Rightarrow \) по теореме Ферма \( g'(a_j) = f_{x_j}'(\ol{a}) = 0 \).
Это верно \( \forall j = 1, \ldots, d \), а значит доказали.

\begin{example}{}{}
    Пусть \( z = |x| + |y| \).
    В точке \( (0, 0) \) не существует частных производных, но \( (0, 0) \) является точкой минимума.

    Если же \( z = x^2 + |y| \), то в точке \( (0, 0) \) определена только одна частная производная,
    однако \( (0, 0) \) снова точка минимума.
\end{example}

\begin{defn}{}{}
    Точки, в которых \( \nabla f = \ol{0} \) будем называть критическими точками.
\end{defn}

\begin{defn}{}{}
    Пусть \( z = x^2 - y^2 \). Тогда в \( (0, 0) \) обе частные производные нулевые,
    однако это не точка минимума.
\end{defn}

\begin{thrm}{Достаточное условие существования экстремума}{}
    Пусть \( f \in D^2 (\ol{a}) \), \( \ol{a} \) --- критическая точка.
    Тогда если:
    \begin{itemize}
        \item
            \( d^2 f(\ol{a}) (\ol{h}) > 0 \: \forall \ol{h} \neq \ol{0} \Rightarrow \) \( \ol{a} \) --- точка минимума.
        \item
            \( d^2 f(\ol{a}) (\ol{h}) < 0 \: \forall \ol{h} \neq \ol{0} \Rightarrow \) \( \ol{a} \) --- точка максимума.
        \item
            Если \( \exists \ol{h}_1 : d^2 f(\ol{a}) (\ol{h}_1) < 0 \)
            и \( \exists \ol{h}_2 : d^2 f(\ol{a}) (\ol{h}_2) > 0 \),
            то \( a \) --- не точка экстремума.
    \end{itemize}
\end{thrm}
...
\begin{gather*}
    f(\ol{a} + \ol{h}) - f(\ol{a})
    =
    \frac{d^2 f(\ol{a}) (\ol{h})}{2} + \alpha (\ol{h}) \cdot || \ol{h} ||^2
    =
    \\
    =
    || \ol{h} ||^2 \left( \frac{ d^2 f(\ol{a}) \left( \frac{\ol{h}}{|| \ol{h} ||} \right) }{2} + \alpha (\ol{h}) \right)^2
    \\
    \lim\limits_{\ol{h} \to \ol{0}} \alpha (\ol{h}) = 0
\end{gather*}

\( \frac{\ol{h}}{ || h || } \in S = \{ \ol{x} \in \RR^d \: \vline \: || \ol{x} || = 1 \} \) --- компакт

Значит \( d^2 f ( \ol{a} ) \) достигает наименьшего (\( m \)) и наибольшего (\( M \)) значений на \( S \).

\begin{enumerate}
    \item
        \( m > 0 \)

        \( \exists \mathring{U} (\ol{0}) : \forall \ol{h} \in \mathring{U} (\ol{0}) \: | \alpha (\ol{h}) | < \frac{m}{3} | \)

        Значит \( f(\ol{a} + \ol{h}) > || \ol{h} ||^2 \left( \frac{m}{2} - \frac{m}{3} \right) > 0 \Rightarrow \) \( a \) --- точка минимума
    \item
        \( M < 0 \)

        \( \exists \mathring{U} (\ol{0}) : \forall \ol{h} \in \mathring{U} (\ol{0}) \: | \alpha (\ol{h}) | < \frac{-M}{3} | \)

        Значит \( f(\ol{a} + \ol{h}) < || \ol{h} ||^2 \left( \frac{M}{2} - \frac{M}{3} \right) < 0 \) \( a \) --- точка максимума
    \item
        \( \exists \ol{h}_1 : d^2 f(\ol{a}) (\ol{h}_1) < 0 \)
        и \( \exists \ol{h}_2 : d^2 f(\ol{a}) (\ol{h}_2) > 0 \)

        \(
            f(\ol{a} + t \ol{h}_1) - f(\ol{a})
            =
            t^2 \left( \frac{d^2 f (\ol{a}) (\ol{h}_1)}{2} + \alpha (t \ol{h}_1) \cdot || \ol{h}_1 ||^2 < 0 \right)
        \)
        при достаточно малых \( t \),
        так как первое слагаемое стремится к отрицательному числу, а второе к нулю.

        Аналогично при малых \( t \) выполнено \( f(\ol{a} + t \ol{h}_2) - f(\ol{a}) > 0 \).

        А это значит, что \( \ol{a} \) не является экстремумом (по одному направлению локальный минимум, а по другому --- локальный максимум).
\end{enumerate}

\begin{example}{}{}
    \( z = x^2 + y^2 \)

    \( 2x = 0, 2y = 0 \Rightarrow x = y = 0 \)

    \( d^2 z (0, 0) = 2 ( (dx)^2 + (dy)^2 ) \)
\end{example}
...
\begin{example}{}{}
    ...
    Значит на \( (1, 0) \) второй дифференциал положителен,
    а на \( (0, 1) \) --- отрицателен.
    Таким образом \( (0, 0) \) не является точкой экстремума.
\end{example}

\begin{example}{}{}
    \( z(x, y) = x^4 + y^4 - 2x^2 + 4xy -2y^2 \)
    ...
    \begin{gather*}
        ...
        0 = x^3 - x - x = x(x^2 - 2) = 0
        \\
        x = y = 0 \quad x = \sqrt{2} = -y \quad x = -\sqrt{2} = -y
    \end{gather*}

    \( f_{xx}'' = 12x^2 - 4 \), \( f_{yy}'' = 12y^2 - 4 \), \( f_{xy}'' = 4 \)

    \( d^2 z ( \pm \sqrt{2}, \mp \sqrt{2} ) = 20 dx^2 + 8 dx dy + 20 dy^2 = 16(dx^2 + dy^2) + 4(dx + dy)^2 \)

    Это выражение \( > 0 \) при \(\forall \ol{h} \neq \ol{0} \)

    Поэтому эти две точки являются точками локального минимума.
    ...
    \(
        ...
        \end{pmatrix} \right|
        > 0
    \) (надо знать критерий Сильвестра) --- при положительном определителе это всегда точка экстремума (для \( 2 \times 2 \)).
    Если \( f_{xx}'' > 0 \), то точка минимума, если \( f_{xx}'' < 0 \), то точка максимума.

    \( d^2 z (0, 0) = - 4 dx^2 + 8 dx dy - 4 dy^2  = -4 (dx - dy)^2 \) --- неопределеннная форма.
    ...
    \( z(x, y) = x^4 + y^4 - 2(x - y)^2 \Rightarrow z(x, x) = 2x^4 \geq 0 \Rightarrow (0, 0) \) --- точка минимума по этому направлению.

    \( z(x, 0) = x^4 - 2x^2 \leq 0 \) при, например, \( x \in \left[ -\frac{1}{2}, \frac{1}{2} \right] \Rightarrow (0, 0) \) может быть только точкой максимума по этому направлению.

    А значит \( (0, 0) \) --- не точка экстремума.
\end{example}
...
\begin{remark}{}{}
    ...
    В случае более больших матриц нужно смотреть на главные миноры.
    Если они все положительные, то это точка минимума.
    Если \( < 0, > 0, < 0, \ldots \),
    то это точка максимума.

    Иначе это либо вырожденная (определитель всей матрицы равен нулю),
    либо неопределенная --- нет экстремума.
\end{remark}

\subsection{Условный экстремум}

\begin{example}{}{}
    Хотим найти экстремумы \( z(x, y) = x^2 - y^2 \) при \( x^2 + y^2 = 1 \).
\end{example}

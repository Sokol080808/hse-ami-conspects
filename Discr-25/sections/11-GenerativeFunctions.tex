...
\subsection{Определение и операции}

\begin{defn}{Производящая функция}{}
    Есть последовательность чисел \( (a_0, a_1, \ldots, a_n, \ldots) \)

    Тогда \( A(x) = a_0 + a_1 x + \ldots + a_n x^n + \ldots = \sum\limits_{n = 0}^{\infty} a_n x^n \) называется ее ПФ.
\end{defn}
...
\begin{itemize}
    ...
    \item
        ...
        \( [AB]_n = \sum\limits_{k = 0}^{n} a_k b_{n - k} \)
    \item
        Деление:

        \begin{lemma}{}{}
            Если \( a_0 \neq 0 \),
            то существует единственная \( B(x) \) такая,
            что \( A(x) B(x) = 1 \)
        \end{lemma}
        ...
        \begin{example}{}{}
            \[
                1 + x + \ldots + x^n + \ldots = \frac{1}{1 - x}
            \]
        \end{example}
\end{itemize}
...
\begin{itemize}
    ...
    \item константная ПФ (умножение на константу эквивалентно умножению на \(1 + 0x + 0x^2 + \ldots \))
    \item \( A(x)B(x) = 0 \Rightarrow A(x) = 0 \lor B(x) = 0 \)
\end{itemize}
...
\begin{defn}{Формальная производная ПФ}{}
    Формальная производная ПФ \( A(x) = \sum\limits_{n = 0}^{\infty} a_n x^n \):
    \begin{gather*}
        A'(x) = \sum\limits_{n = 1}^{\infty} n a_n x^{n - 1}
    \end{gather*}
\end{defn}
...
Левая часть при \( [x^n] \):
\begin{gather*}
    (n+1) \sum\limits_{k = 0}^{n + 1} a_k b_{n + 1 - k}
\end{gather*} 

Правая часть при \( [x^n] \):
\begin{gather*}
    \sum\limits_{t = 0}^{n} a_{t + 1} b_{n - t} + \sum\limits_{s = 0}^{n} a_{n - s} b_{s + 1}
\end{gather*} 

Замена \( s = n - t \)
\begin{gather*}
    (n + 1) a_{n + 1} b_0 + n a_n b_1 + (n - 1) a_{n - 1} b_2 + \ldots
    +
    \\
    +
    (n + 1) a_0 b_{n + 1} + n a_1 b_n + (n - 1) a_2 b_{n - 1} + \ldots
\end{gather*} 
...
\begin{example}{}{}
    \begin{gather*}
        A(x) = 1 + x + x^2 + \ldots
        \\
        (1 + x + x^2 + \ldots)(1 - x) = 1 \Rightarrow A(x) = \dfrac{1}{1 - x} = (1 - x)^{-1}
    \end{gather*}
\end{example}
...
\begin{example}{Сумма первых \( n \) натуральных чисел}{}
    \begin{gather*}
        (1, 1, 1, \ldots) \longrightarrow \dfrac{1}{1 - x}
        \\
        (1, 2, 3, \ldots) \longrightarrow \left( \dfrac{1}{1 - x} \right)' = \dfrac{1}{(1 - x)^2}
        \\
        (1, 1 + 2, 1 + 2 + 3, \ldots) \longrightarrow \left( \dfrac{1}{(1 - x)^2} \right)' = \dfrac{1}{(1 - x)^3}
        \\
        (0, 0 + 1, 0 + 1 + 2, \ldots) \longrightarrow \dfrac{x}{(1 - x)^3}
        \\
        a_n = 0 + 1 + \ldots + n
        \\
        A(x) = \frac{x}{(1 - x)^3}
...
\begin{gather*}
    1 - \sqrt{1 - 4x} = 1 - \sum\limits_{n=0}^{\infty} \binom{1/2}{n} (-4x)^n
    =
    \\
    =
    -\sum\limits_{n=1}^{\infty} \dfrac{\frac{1}{2} \cdot \left( \frac{1}{2} - n + 1 \right) \cdot \ldots
        \cdot \left( \frac{1}{2} - 1 \right)}{n!} (-4x)^n
    =
    \\
    =
    -\sum\limits_{n=1}^{\infty} \dfrac{1 (-1) (-3) \ldots (-2n + 3)}{2^n \cdot k!} (-4x)^n
    =
    \\
    =
    \sum\limits_{n=1}^{\infty} \dfrac{1 \cdot 2 \cdot 3 \cdot \ldots \cdot (2n - 3)}{2^n \cdot n!} 4^n x^n
    =
    \sum\limits_{n=1}^{\infty} \dfrac{(2n - 2)! \cdot 2^n}{n! \cdot 2 \cdot 4 \cdot \ldots \cdot (2n-2)} x^n
    =
    \\
    =
    \sum\limits_{n=1}^{\infty} \dfrac{(2n - 2)! \cdot 2}{n! \cdot (n - 1)!} x^n
    \\
    C(x)
    =
    \dfrac{1 - \sqrt{1 - 4x}}{2x}
    =
    \sum\limits_{n=1}^{\infty} \dfrac{(2n - 2)!}{n! \cdot (n - 1)!} x^{n-1}
    =
    \sum\limits_{n=0}^{\infty} \dfrac{(2n)!}{(n + 1) \cdot (n!)^2} x^{n}
\end{gather*}
